...
import { z } from "zod";
import { updateBeneficiaryFieldValue, createBeneficiaryFieldValue } from "@/app/api/beneficiaries/actions";
...
export interface BeneficiaryFieldValue {
  id: string;
...
interface BeneficiaryWithFields {
  id: string;
...
interface BeneficiaryEditModalProps {
  ...
  beneficiary: BeneficiaryWithFields | null;
...
export function BeneficiaryEditModal({
  open,
  onOpenChange,
  beneficiary,
  customFields,
  ...
  useEffect(() => {
    if (beneficiary && open && !formInitializedRef.current) {
  ...
  async function onSubmit(data: FormValues) {
    ...
    try {
      // Update existing field values or create new ones
      const updatePromises = data.fieldValues.map(async fieldValue => {
        ...
        } else if (fieldValue.value !== null && fieldValue.value !== '') {
          // Create new field value for existing beneficiary if value is not empty
          return createBeneficiaryFieldValue(
            beneficiary.id,
            fieldValue.custom_field_id,
            fieldValue.value,
            orgId
          );
        }
              ...
              <FormItem>
                ...
                <FormControl>
                  <Input 
                    ...
                    data-testid={`modal-field-${field.id}`}
                  />
                </FormControl>
                <FormMessage />
              </FormItem>
          ...
          <FormField
            ...
            render={({ field: formField }) => (
              <FormItem>
                ...
                >
                  <FormControl>
                    <SelectTrigger data-testid={`modal-field-${field.id}`}>
  ...
  const handleOpenChange = (newOpenState: boolean) => {
    // If the modal is closing, reset form
    if (!newOpenState) {
      form.reset();
    }
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent 
        ...
        data-testid="beneficiary-edit-modal"
      >
        <DialogHeader>
          ...
          <DialogDescription>
            Update information for this beneficiary
          </DialogDescription>
        </DialogHeader>
