...
docker logs supabase_db_cursordevkit_riseloop 2>&1 | grep -i "duration:" | grep -i "ms)" | awk '{ if ($NF > 1000) print $0 }' | tail -n 20 | while read -r line; do
    echo -e "${YELLOW}$line${NC}"
done

# Database Errors & Warnings
print_header "Database Logs (Errors & Warnings)"
docker logs supabase_db_cursordevkit_riseloop --tail 50 2>&1 | grep -i "error\|warning\|notice\|fatal" | while read -r line; do
    if [[ $line =~ ERROR|FATAL ]]; then
        echo -e "${RED}$line${NC}"
...
docker logs supabase_auth_cursordevkit_riseloop --tail 50 2>&1 | grep -i "error\|failed\|success.*login\|token" | tail -n 20 | while read -r line; do
...
docker logs supabase_rest_cursordevkit_riseloop --tail 20 2>&1 | while read -r line; do
    if [[ $line =~ error|failed ]]; then
        echo -e "${RED}$line${NC}"
    elif [[ $line =~ warning ]]; then
        echo -e "${YELLOW}$line${NC}"
...
docker logs supabase_realtime_cursordevkit_riseloop --tail 20 2>&1 | while read -r line; do
    if [[ $line =~ error|failed ]]; then
        echo -e "${RED}$line${NC}"
    elif [[ $line =~ warning ]]; then
        echo -e "${YELLOW}$line${NC}"
...
docker logs supabase_storage_cursordevkit_riseloop --tail 20 2>&1 | while read -r line; do
    if [[ $line =~ error|failed ]]; then
        echo -e "${RED}$line${NC}"
    elif [[ $line =~ warning ]]; then
        echo -e "${YELLOW}$line${NC}"
