 ...
 * ```
 */
export async function createTeam({ supabase, team, }) {
    try {
        ...
        throw new TeamOperationError("Create Team", "Failed to create new team", "Unable to create team. Please try again.", SupabaseErrorCode.CREATE_FAILED, error);
    }
}
/**
...
export async function getTeam({ supabase, teamId, }) {
    try {
        ...
        throw new TeamOperationError("Get Team", `Failed to retrieve team with ID: ${teamId}`, "Unable to load team details. Please refresh the page.", SupabaseErrorCode.READ_FAILED, error);
    }
}
/**
 ...
 * ```
 */
export async function getTeams({ supabase, organizationId, }) {
    try {
        const { data, error } = await supabase
            .from("teams")
            .select("*")
        ...
        throw new TeamOperationError("Get Teams", `Failed to retrieve teams for organization: ${organizationId}`, "Unable to load teams. Please refresh the page.", SupabaseErrorCode.READ_FAILED, error);
    }
}
/**
 ...
 * ```
 */
export async function updateTeam({ supabase, teamId, team, }) {
    try {
        const { data, error } = await supabase
            .from("teams")
            .update(team)
        ...
        throw new TeamOperationError("Update Team", `Failed to update team with ID: ${teamId}`, "Unable to update team details. Please try again.", SupabaseErrorCode.UPDATE_FAILED, error);
    }
}
/**
...
export async function deleteTeam({ supabase, teamId, }) {
    try {
        ...
        throw new TeamOperationError("Delete Team", `Failed to delete team with ID: ${teamId}`, "Unable to delete team. Please try again.", SupabaseErrorCode.DELETE_FAILED, error);
    }
}
