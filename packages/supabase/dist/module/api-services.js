 ...
 * ```
 */
const getApiServices = async ({ supabase, }) => {
 ...
 * ```
 */
const getUserApiQuotas = async ({ supabase, userId, }) => {
 ...
 * ```
 */
const updateApiQuota = async ({ supabase, userId, serviceId, dailyQuota, queriesPerSecond, }) => {
    const { data, error } = await supabase
        .from('api_quota_allocations')
        .upsert({
        user_id: userId,
