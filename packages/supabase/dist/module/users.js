...
export async function createUser({ supabase, user, }) {
    try {
        ...
        throw new UserOperationError("Create user", "Failed to create new user", "Unable to create user. Please try again.", SupabaseErrorCode.CREATE_FAILED, error);
    }
}
/**
 ...
 * ```
 */
export async function getUser({ supabase, userId, }) {
    try {
        const { data, error } = await supabase
            .from("users")
            .select("*")
        ...
        throw new UserOperationError("Get user", `Failed to retrieve user with ID: ${userId}`, "Unable to load user details. Please refresh the page.", SupabaseErrorCode.READ_FAILED, error);
    }
}
/**
...
export async function getUsers({ supabase, }) {
    try {
        ...
        throw new UserOperationError("Get users", "Failed to retrieve users list", "Unable to load users. Please refresh the page.", SupabaseErrorCode.READ_FAILED, error);
    }
}
/**
 ...
 * ```
 */
export async function updateUser({ supabase, userId, user, }) {
    try {
        const { data, error } = await supabase
            .from("users")
            .update(user)
        ...
        throw new UserOperationError("Update user", `Failed to update user with ID: ${userId}`, "Unable to update user details. Please try again.", SupabaseErrorCode.UPDATE_FAILED, error);
    }
}
/**
...
export async function deleteUser({ supabase, userId, }) {
    try {
        ...
        throw new UserOperationError("Delete user", `Failed to delete user with ID: ${userId}`, "Unable to delete user. Please try again.", SupabaseErrorCode.DELETE_FAILED, error);
    }
}
