 ...
 */
export async function createTestMember({ organizationId, teamId, userId, isAdmin = false, membershipType = "team", roles, }) {
    ...
    if (!orgMember)
        throw new Error("No organization member data returned");
    ...
    if (!teamMember)
        throw new Error("No team member data returned");
    return {
 ...
 */
export async function createTestMembers({ organizationId, teamId, members, roles, }) {
    return Promise.all(members.map((member) => createTestMember({
        organizationId,
        teamId,
