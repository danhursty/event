 ...
 */
export async function loginTestUser({ email, password = "password123", }) {
    const adminClient = createAdminClient();
 ...
 */
export async function loginTestUsers({ users, }) {
    return Promise.all(users.map((user) => loginTestUser({
        email: user.email,
